...
class KPipeline:
    ...
    2. Manage and store voices, loaded from local path or lazily downloaded from HF if needed
    ...
    def __init__(
        ...
        local_voices_dir: Optional[str] = DEFAULT_LOCAL_VOICES_DIR  # NEW
    ):
        ...
        Args:
            ...
            local_voices_dir: Local directory to load voice .pt files from.
                              Set to None to always download from HuggingFace.
        """
        if repo_id is None:
            repo_id = 'hexgrad/Kokoro-82M'
            print(f"WARNING: Defaulting repo_id to {repo_id}. Pass repo_id='{repo_id}' to suppress this warning.")
        self.repo_id = repo_id
        self.local_voices_dir = local_voices_dir  # NEW
    ...
    def load_single_voice(self, voice: str):
        ...
        # Explicit .pt path — use directly
        if voice.endswith('.pt'):
            f = voice

        # Try local voices directory first
        elif self.local_voices_dir:
            local_path = os.path.join(self.local_voices_dir, f'{voice}.pt')
            if os.path.isfile(local_path):
                logger.debug(f"Loading voice from local path: {local_path}")
                f = local_path
            else:
                # File missing locally — fall back to HF download
                logger.warning(f"Voice '{voice}' not found at {local_path}, falling back to HuggingFace download.")
                from huggingface_hub import hf_hub_download
                f = hf_hub_download(repo_id=self.repo_id, filename=f'voices/{voice}.pt')

        # No local dir configured — always download from HF
        else:
            from huggingface_hub import hf_hub_download
            f = hf_hub_download(repo_id=self.repo_id, filename=f'voices/{voice}.pt')
